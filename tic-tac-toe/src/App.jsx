      ...
      <div className="grid">
        {[...Array(9)].map((_, index) => (
            <div className="grid-item" key={index}>
              <button className={board[index] === "" ? "square" : board[index] === "X" ? "square button-X" : "square button-O"} onClick={() => handleClick(index + 1)}>{board[index]}</button>
            </div>
          ))}
      </div>

      <StatusText currentPlayer={currentPlayer} isWinner={winner} isTie={tieGame}/>
      
      <button className='reset reset-button' onClick={() => {onResetClick()}}>
        Reset
      </button>
    </>
  )
